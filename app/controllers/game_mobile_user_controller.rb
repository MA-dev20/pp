class GameMobileUserController < ApplicationController
  before_action :authenticate_game!, :set_game, only: [:wait, :choose, :turn, :play, :rate, :rated, :rating, :bestlist, :ended, :reject_user ,:accept_user, :welcome, :new, :new_name, :new_company, :new_avatar, :new_turn, :bestlist]
  ...
  def accept_user
    ...
    if @admin.plan_type.eql?('trial')
      ...
      create_turn_against_user(@user, @admin, "accepted")
      ActionCable.server.broadcast  "count_#{@game1.id}_channel", count: 'true', counter: @game.turns.where(status: "accepted").playable.count.to_s, modal: false, user_id: params[:user_id], new: 'true', user_pic: @user.avatar.quad.url
